...
var DOQueueHandler = class extends DurableObject {
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "Mv9BQ6l_NEuVRW03NYp79");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "Mv9BQ6l_NEuVRW03NYp79");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "Mv9BQ6l_NEuVRW03NYp79");
