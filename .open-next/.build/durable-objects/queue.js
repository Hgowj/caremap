...
var DOQueueHandler = class extends DurableObject {
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "mDQry0NUCqSxC4CfGQ1_a");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "mDQry0NUCqSxC4CfGQ1_a");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "mDQry0NUCqSxC4CfGQ1_a");
