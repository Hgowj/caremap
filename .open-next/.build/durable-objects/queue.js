...
var DOQueueHandler = class extends DurableObject {
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "Zem_cmDkX53R3sBSX6QDk");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "Zem_cmDkX53R3sBSX6QDk");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "Zem_cmDkX53R3sBSX6QDk");
