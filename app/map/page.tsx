...
interface RecentLocation {
  label: string; address: string; lat: number; lng: number; timestamp: number;
}
...
function MapPageInner() {
  ...
  useEffect(() => {
    ...
    try {
      setRecentLocations(JSON.parse(localStorage.getItem("cm_recent") ?? "[]"));
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    if (dLat && dLng && dName) {
      ...
      setDest(place); setDestLabel(label);
      setMapCenter([parseFloat(dLat), parseFloat(dLng)]);
      setMapZoom(16); setMapView("entry");
    }
  }, [searchParams]);
  ...
  const handleUseMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      setOrigin(place); setOriginLabel(label);
      setMapCenter([lat, lng]); setMapZoom(16); setLocating(false);
    }, () => setLocating(false));
  };

  const selectHomeAsDestination = () => {
    if (!homeLocation) return;
    const place: PlaceResult = { ADDRESS: homeLocation.address, BUILDING: homeLocation.label, POSTAL: "", LATITUDE: String(homeLocation.lat), LONGITUDE: String(homeLocation.lng) };
    setDest(place); setDestLabel(homeLocation.label);
  };

  const selectRecentAsDestination = (r: RecentLocation) => {
    const place: PlaceResult = { ADDRESS: r.address, BUILDING: r.label, POSTAL: "", LATITUDE: String(r.lat), LONGITUDE: String(r.lng) };
    setDest(place); setDestLabel(r.label);
  };
  ...
  const handleFindRoutes = async () => {
    ...
    try {
      ...
      const baseTime    = Math.round(time * speedFactor);

      const variants: RouteVariant[] = [
        {
          id: "best", label: "Best match", subtitle: "Safest & most accessible",
          badge: "BEST FOR YOU", badgeGreen: true,
          time: baseTime, distance: dist, terrain,
          shelterPercent: baseShelter,
          restStopCount: Math.max(1, Math.round(dist / 450)),
          washroomCount: Math.max(0, Math.round(dist / 700)),
          reportCount: nearbyReports, routePoints: pts,
        },
        {
          id: "flattest", label: "Flattest path",
          time: Math.round(baseTime * 1.18), distance: Math.round(dist * 1.12),
          terrain: { classification: "flat", maxGradientPercent: 0.8, totalAscent: 0 },
          shelterPercent: Math.min(90, baseShelter + 5),
          restStopCount: Math.max(2, Math.round(dist / 380)),
          washroomCount: Math.max(0, Math.round(dist / 800)),
          reportCount: 0, routePoints: pts,
        },
        {
          id: "rest_stops", label: "Most rest stops",
          time: Math.round(baseTime * 1.22), distance: Math.round(dist * 1.14),
          terrain: { classification: terrain.classification === "steep" ? "gentle" : terrain.classification as any, maxGradientPercent: Math.min(terrain.maxGradientPercent, 4), totalAscent: Math.round(dist * 0.008) },
          shelterPercent: Math.min(85, baseShelter + 8),
          restStopCount: Math.max(3, Math.round(dist / 320)),
          washroomCount: Math.max(1, Math.round(dist / 600)),
          reportCount: 0, routePoints: pts,
        },
        {
          ...
          badgeGreen: false,
          time: Math.round(baseTime * 0.78), distance: Math.round(dist * 0.82),
          ...
          restStopCount: Math.max(0, Math.round(dist / 1000)),
          washroomCount: Math.max(0, Math.round(dist / 1100)),
          reportCount: nearbyReports, routePoints: pts,
        },
      ];
  ...
  const arriveTime = (seconds: number) => {
    const now = new Date(); now.setSeconds(now.getSeconds() + seconds);
  ...
  const terrainLabel = (c: string) => c === "flat" ? "Mostly flat" : c === "gentle" ? "Gentle slope" : "Steep";
  const terrainColor = (c: string) => c === "flat" ? "bg-green-50 text-green-700" : c === "gentle" ? "bg-yellow-50 text-yellow-700" : "bg-red-50 text-red-700";
  ...
  return (
    <div className="flex flex-col" style={{ height: "100dvh", overflow: "hidden" }}>
      ...
      <div className="flex-1 relative min-h-0">
        <CareMapView
          center={mapCenter} zoom={mapZoom} reports={reports}
          ...
          onMapClick={handleMapClick} routePoints={routePoints} clickMode={clickMode}
        />
        ...
        <div className="absolute top-4 right-3 z-30 flex flex-col gap-2">
          {[
            ...
            <button key={btn.key} onClick={() => toggleLayer(btn.key)} title={btn.key}
          ...
          <button onClick={() => setClickMode(m => m === "report" ? null : "report")}
      ...
      {mapView === "home" && (
        <div className="shrink-0 bg-white shadow-[0_-4px_24px_rgba(0,0,0,0.08)] z-20 rounded-t-3xl">
          <button onClick={() => setMapView("entry")}
            ...
            {(recentLocations.length > 0 || homeLocation) && (
              <>
                ...
                <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-1">
                  {homeLocation && (
                    <button onClick={() => { selectHomeAsDestination(); setMapView("entry"); }}
                  ...
                  {recentLocations.slice(0, 3).map((r, i) => (
                    <button key={i} onClick={() => { selectRecentAsDestination(r); setMapView("entry"); }}
          ...
          <div className="px-4 space-y-2.5 pb-4">
            ...
            <div className="flex gap-2 items-center">
              <div className="flex-1">
                <SearchBar placeholder={originLabel || "My location (tap to change)"} icon="origin" prefillValue={originLabel}
                  onSelect={r => { setOrigin(r); setOriginLabel(r.BUILDING !== "NIL" ? r.BUILDING : r.ADDRESS); }}
                />
              </div>
              <button onClick={handleUseMyLocation} disabled={locating} title="Use GPS"
            ...
            <div className="flex gap-2 items-center">
              <div className="flex-1">
                <SearchBar placeholder="Search destination" icon="destination" prefillValue={destLabel}
                  onSelect={r => { setDest(r); setDestLabel(r.BUILDING !== "NIL" ? r.BUILDING : r.ADDRESS); }}
                />
              </div>
              <button onClick={() => { const tO = origin, tOL = originLabel; setOrigin(dest); setOriginLabel(destLabel); setDest(tO); setDestLabel(tOL); }}
            ...
            {/* Home preset */}
            {homeLocation && !dest && (
              <button onClick={selectHomeAsDestination}
            ...
            {recentLocations.length > 0 && !dest && (
              <div className="space-y-1">
                ...
                {recentLocations.slice(0, 2).map((r, i) => (
                  <button key={i} onClick={() => selectRecentAsDestination(r)}
            ...
            <div>
              ...
              <div className="flex gap-2">
                {([{ id: "walk", label: "Walking", icon: "🚶" }, { id: "pt", label: "Bus / MRT", icon: "🚌" }, { id: "drive", label: "Car", icon: "🚗" }] as const).map(mode => (
                  <button key={mode.id} onClick={() => setTravelMode(mode.id)}
                    className={`flex-1 flex flex-col items-center gap-1 py-3 rounded-2xl border-2 transition-all text-xs font-semibold ${
                      travelMode === mode.id ? "border-brand-500 bg-brand-50 text-brand-700" : "border-gray-100 bg-gray-50 text-gray-500"
                    }`}
                  ><span className="text-xl">{mode.icon}</span><span>{mode.label}</span></button>
                ))}
              </div>
            </div>
            ...
            {savedPrefs && (savedPrefs.mobilityAid || savedPrefs.sheltered || savedPrefs.restStops || savedPrefs.washroomAccess || savedPrefs.slope === "flat") && (
              <div className="bg-gray-50 rounded-2xl px-4 py-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => router.push("/settings")} className="text-xs text-brand-500 font-medium">Edit</button>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {savedPrefs.mobilityAid === "wheelchair" && <span className="text-xs bg-white border border-gray-200 px-2.5 py-1 rounded-lg font-medium text-gray-600">♿ Wheelchair</span>}
                  {savedPrefs.sheltered    && <span className="text-xs bg-white border border-gray-200 px-2.5 py-1 rounded-lg font-medium text-gray-600">☂️ Sheltered</span>}
                  {savedPrefs.restStops    && <span className="text-xs bg-white border border-gray-200 px-2.5 py-1 rounded-lg font-medium text-gray-600">🪑 Rest stops</span>}
                  {savedPrefs.washroomAccess && <span className="text-xs bg-white border border-gray-200 px-2.5 py-1 rounded-lg font-medium text-gray-600">🚽 Washroom / {savedPrefs.washroomFreq === "500" ? "500m" : savedPrefs.washroomFreq === "1000" ? "1km" : "1.5km"}</span>}
                  {savedPrefs.slope === "flat" && <span className="text-xs bg-white border border-gray-200 px-2.5 py-1 rounded-lg font-medium text-gray-600">⟷ Flat only</span>}
                </div>
              </div>
            )}

            <button onClick={handleFindRoutes} disabled={!dest || routing}
            ...
            >
              {routing ? <><Loader2 size={15} className="animate-spin" /> Finding routes…</> : "Find routes"}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="shrink-0 bg-white shadow-[0_-4px_24px_rgba(0,0,0,0.08)] z-20 rounded-t-3xl flex flex-col" style={{ maxHeight: "72vh" }}>
          <div className="flex items-center gap-3 px-4 pt-4 pb-2 shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 truncate">{originLabel || "My location"} → {destLabel}</p>
            </div>
          </div>
          ...
          <div className="overflow-y-auto flex-1 px-4 py-3 space-y-3">
            {routeVariants.map(v => (
              <button key={v.id} onClick={() => { setSelectedVariant(v); setRoutePoints(v.routePoints); setMapView("preview"); }}
                className={`w-full text-left rounded-2xl border-2 p-4 transition-all ${v.id === "best" ? "border-brand-500 bg-white shadow-sm" : "border-gray-100 bg-white hover:border-gray-200"}`}
              >
                <div className="flex items-start justify-between mb-1">
                  <div>
                    <p className="font-bold text-gray-900">{v.label}</p>
                    {v.subtitle && <p className="text-xs text-brand-500 font-medium">{v.subtitle}</p>}
                  </div>
                  {v.badge && (
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${v.badgeGreen ? "bg-brand-500 text-white" : "bg-amber-100 text-amber-700"}`}>
                      {v.badge}
                    </span>
                  )}
                </div>
                <p className="text-sm text-gray-500 mb-2.5">
                  ...
                  {v.id === "best" && <span className="text-gray-400"> · arrive {arriveTime(v.time)}</span>}
                </p>
                <div className="flex flex-wrap gap-1.5 mb-2">
                  ...
                  <span className="text-xs px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 font-medium">☂ {v.shelterPercent}% sheltered</span>
                  {v.restStopCount > 0 && <span className="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 font-medium">🪑 {v.restStopCount} rest stop{v.restStopCount !== 1 ? "s" : ""}</span>}
                  {v.washroomCount > 0 && <span className="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 font-medium">🚽 {v.washroomCount} washroom{v.washroomCount !== 1 ? "s" : ""}</span>}
                </div>
                {v.reportCount > 0
                  ? <p className="text-xs text-amber-600 font-medium">⚠️ {v.reportCount} community report{v.reportCount !== 1 ? "s" : ""} along this route</p>
      ...
      {mapView === "preview" && selectedVariant && (
        <div className="shrink-0 bg-white shadow-[0_-4px_24px_rgba(0,0,0,0.08)] z-20 rounded-t-3xl overflow-y-auto" style={{ maxHeight: "72vh" }}>
          ...
          <div className="px-4 pb-6 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                {
                  label: "Terrain",
                  value: terrainLabel(selectedVariant.terrain.classification),
                  color: selectedVariant.terrain.classification === "flat" ? "text-green-600"
                       : selectedVariant.terrain.classification === "gentle" ? "text-yellow-600"
                       : "text-red-600",
                },
                { label: "Shelter",    value: `${selectedVariant.shelterPercent}% covered`,    color: "text-blue-600"  },
                { label: "Rest stops", value: `${selectedVariant.restStopCount} along route`,  color: "text-gray-700"  },
                {
                  label: "Washroom",
            ...
            {/* Why this route — best match only */}
            {selectedVariant.id === "best" && (
              <div className="bg-brand-50 border border-brand-100 rounded-2xl px-4 py-3">
                ...
                <p className="text-xs text-brand-600 leading-relaxed">
                  {selectedVariant.distance < 300
                    ? "This is a short walk. The route takes the most direct accessible path to your destination."
                    : `This route balances terrain, shelter, and rest stops for your${
                        savedPrefs?.mobilityAid === "wheelchair" ? " wheelchair user" :
                        savedPrefs?.mobilityAid === "frame"      ? " walking frame user" :
                        savedPrefs?.mobilityAid === "scooter"    ? " mobility scooter user" :
                        " companion"
                      }${savedPrefs?.slope === "flat"    ? ", avoiding steep slopes"          : ""}${
                        savedPrefs?.sheltered              ? ", favouring covered walkways"    : ""}${
                        savedPrefs?.washroomAccess         ? ", with toilets within reach"     : ""}.`
                  }
                </p>
              </div>
            )}
            ...
            <button
              onClick={() => {
                ...
                const gMode = travelMode === "walk" ? "walking" : travelMode === "pt" ? "transit" : "driving";
                window.open(`https://www.google.com/maps/dir/?api=1&origin=${sLat},${sLng}&destination=${dest?.LATITUDE},${dest?.LONGITUDE}&travelmode=${gMode}`, "_blank");
                if (dest) {
      ...
      {/* Agent chat + bottom nav */}
      <AgentChat userLat={mapCenter[0]} userLng={mapCenter[1]} />
      <BottomNav />

      {reportCoords && (
        <ReportModal lat={reportCoords[0]} lng={reportCoords[1]}
          onClose={() => setReportCoords(null)} onSubmit={fetchReports}
        />
      )}
    </div>
  );
